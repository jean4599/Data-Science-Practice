...
int main(int argc, const char *argv[])
{
    ...
    // test_2();
    // test_3();
    uint64_t min_support_percent = 0.1;

    // Input Data and Build FList
    freopen("sample.in", "r", stdin);
    std::vector<vector<Item> > transactions;
    std::string line;
    ...
    while(!getline(cin,line).eof()){
        ...
        while(getline(ssline, number,',')){
            string item = number.c_str();
            arr.push_back(item);
        }
        transactions.push_back(arr);
    }
    uint64_t min_support = min_support_percent * transactions.size();

    FPTree fptree(transactinos, min_support);
    fptree.preorder_print();
    return EXIT_SUCCESS;
}
